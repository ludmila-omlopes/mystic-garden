...
import { useProfiles, useExploreProfiles, ExploreProfilesOrderByType, LimitType, profileId, Profile } from '@lens-protocol/react-web';
...
export default function SearchPage() {
  ...
  const [profile, setProfile] = useState<Profile>(); // Add state for profile
  ...
  useEffect(() => {
    // Update profile when profiles or profileByHandle data changes
  ...
  return (
    ...
    ">
      ...
      <div className="mt-4">
        <input type="text" value={searchValue} onChange={e => setSearchValue(e.target.value)} placeholder={`Filter by ${searchType}`} className="px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
      </div>
